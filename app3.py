...
from respondent_cpy import ask_question
...
with st.sidebar:
    with st.expander("Upload Document(s)", expanded=False):
        uploaded_files = st.file_uploader(
            ...
            type=["pdf", "docx", "xlsx", "pptx", "xls", "msg", "eml"],
...
if prompt := st.chat_input("Ask your question here...") and st.session_state.documents:
    # Display the user's question in the chat
    st.session_state.chat_history.append({"role": "user", "content": prompt})

    with st.spinner("Thinking..."):
        inline_prompt = f"Given the following context, answer the question: Question: {prompt}\nAnswer:"
